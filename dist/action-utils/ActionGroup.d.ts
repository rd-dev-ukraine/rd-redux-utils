...
export interface ActionGroup<TCommonProps, TExtraActions extends Action = never> {
     ...
     */
    includeAction<TAction extends Action>(test: (action: Action) => action is TAction, extractProps: (action: TAction) => TCommonProps | true): ActionGroup<TCommonProps, TExtraActions | TAction>;
    /**
     * Creates a hash reducer function which processes the actions of this group and subgroups.
     */
    hashedReducer<TState>(keySelector: (props: TCommonProps) => string, elementReducer: Reducer<TState>): Reducer<StateHash<TState>>;
    hashedReducerWithSelector<TState>(keySelector: (props: TCommonProps) => string, elementReducer: Reducer<TState>): {
        reducer: Reducer<StateHash<TState>>;
        selector: (state: StateHash<TState>, params: TCommonProps) => TState | undefined;
    };
    indexedReducer<TState>(indexSelector: (props: TCommonProps) => number, elementReducer: Reducer<TState>): Reducer<TState[]>;
}
